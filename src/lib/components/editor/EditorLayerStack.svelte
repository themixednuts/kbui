<script lang="ts">
  ...
  const layerButtonClass =
    "layer-button h-kb-34 min-w-0 flex-none gap-kb-8 rounded-pill border-line-2 bg-surface px-kb-14 py-0 [font-family:var(--mono)] text-[11px] tracking-[0.04em] font-strong text-ink-2 shadow-card transition-[background,border-color,color,transform] duration-[var(--dur-fast)] ease-[var(--ease-out-soft)] hover:-translate-y-px hover:border-line-2 hover:bg-surface-2 hover:text-ink";
...
<div class={layerStackClass} role="group" aria-label="Keyboard layers">
  ...
  {#if allowAdd}
    <Button
      variant="ghost"
      size="sm"
      class="add-layer h-kb-34 flex-none gap-kb-6 rounded-pill px-kb-12 font-mono text-[11px] tracking-[0.04em]"
      title="Add transparent layer"
      aria-label="Add transparent layer"
      onclick={() => editor.addLayer()}
    >
      <Plus size={14} aria-hidden="true" />
      <span class="max-[520px]:hidden">Layer</span>
    </Button>
  {/if}
</div>

<script lang="ts">
  ...
  const previewPlaneClass =
    "preview-plane grid min-h-0 min-w-0 place-items-center !border-b !border-line ![background-image:linear-gradient(to_right,oklch(0.13_0.01_60_/_0.045)_1px,transparent_1px),linear-gradient(to_bottom,oklch(0.13_0.01_60_/_0.045)_1px,transparent_1px)] ![background-color:color-mix(in_oklch,var(--paper-2)_76%,var(--surface))] p-kb-16 [background-size:18px_18px] hover:[&_.preview-key.lit]:-translate-y-px";
  ...
  const previewKeyClass =
    "preview-key aspect-[1.45] min-w-0 rounded-[4px] border border-[rgba(24,22,20,0.12)] [background:linear-gradient(180deg,#fffdf7_0%,#e9dfca_100%)] shadow-[0_1px_0_rgba(0,0,0,0.12)] transition-transform duration-[var(--dur-fast)] ease-[var(--ease-out-soft)]";
  const previewKeyLitClass =
    "lit border-[color-mix(in_oklch,var(--key-c)_60%,rgba(24,22,20,0.14))] [background:color-mix(in_oklch,var(--key-c)_54%,#fffdf7)]";
  ...
  const avatarClass =
    "avatar grid size-kb-24 shrink-0 place-items-center rounded-[7px] border border-[color-mix(in_oklch,var(--coral)_42%,var(--line-2))] bg-coral text-[9px] font-bold text-[#1c0a04]";
  ...
  const tagClass =
    "tag min-h-kb-24 rounded-pill !border !border-line !bg-paper-2 px-kb-8 py-0 !font-mono !text-[10px] !text-ink-2";
